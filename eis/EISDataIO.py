...
def eis_dataframe_from_csv(csv_path: FilePath | ReadCsvBuffer[bytes] | ReadCsvBuffer[str]) -> pd.DataFrame:
...
def eis_dataframe_to_csv(df: pd.DataFrame, path: FilePath | WriteBuffer[str] | WriteBuffer[bytes]) -> None:
